...
import { Transaction, BudgetGoal, View, CategoryType } from './types';
...
const App: React.FC = () => {
  ...
  const [transactions, setTransactions] = useState<Transaction[]>(() => {
    ...
    // Ensure we only load expenses if any income was previously stored
    const data = saved ? JSON.parse(saved) : INITIAL_TRANSACTIONS;
    return data.filter((t: any) => t.type === 'expense');
  });
  ...
  const addTransaction = (newTx: Omit<Transaction, 'id'>) => {
    const transaction = { ...newTx, id: Math.random().toString(36).substr(2, 9), type: 'expense' as const };
      ...
      <div className="p-6 pt-10 pb-20">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => { setScannedData(null); setIsModalOpen(true); }} className="w-10 h-10 flex items-center justify-center bg-red-500 text-white rounded-full shadow-lg active:scale-90 transition-transform">
        ...
        {activeView === View.DASHBOARD && (
          <div className="space-y-6">
            <div className="bg-gradient-to-br from-red-600 to-rose-700 p-6 rounded-[2rem] text-white shadow-xl">
              <p className="text-red-100 text-sm font-medium mb-1">Total de Gastos (Mês)</p>
              <h2 className="text-4xl font-bold mb-2">${totalExpenses.toLocaleString()}</h2>
              <p className="text-red-200 text-xs">Acompanhando seus gastos mensais com inteligência.</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              >
                ...
                <span className="font-bold text-gray-800 text-sm">Novo Gasto</span>
              </button>
              <button 
              ...
              >
                ...
                <span className="font-bold text-gray-800 text-sm">Scan Recibo</span>
              </button>
            </div>

            <div>
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-bold">Últimos Gastos</h3>
                ...
                {transactions.slice(0, 5).map(t => {
                   ...
                   return (
                    <div key={t.id} className="flex items-center justify-between bg-white p-3 rounded-2xl border border-gray-50">
                      ...
                      <span className="font-bold text-sm text-gray-900">-${t.amount}</span>
                    </div>
                   );
                })}
              </div>
              ...
              return (
                <div key={t.id} className="group flex items-center justify-between bg-white p-4 rounded-2xl shadow-sm border border-gray-100 transition-all hover:border-blue-200">
                  ...
                  <div className="flex flex-col items-end">
                    <span className="font-bold text-lg text-gray-900">-${t.amount.toLocaleString()}</span>
        ...
        {activeView === View.STATS && (
          <div className="space-y-6">
               ...
               <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {group1.map(cat => {
                      ...
                      const real = transactions.filter(t => t.category === cat).reduce((s, t) => s + t.amount, 0);
            ...
            <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100">
              <h3 className="text-lg font-bold mb-6 text-center">Distribuição por Categoria</h3>
        ...
        {activeView === View.AI_ADVISOR && (
          <div className="space-y-6">
            <div className="bg-gradient-to-br from-indigo-500 to-purple-600 p-8 rounded-[2.5rem] text-white shadow-xl text-center relative overflow-hidden">
              ...
              <p className="text-indigo-100 text-xs mb-6">Analise seus gastos para economizar mais.</p>
